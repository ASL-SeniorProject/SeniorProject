...
class Guesser(object):
	def __init__(self, type="multi", name="tempNet"):
		...
		if type == "multi":
			...
			self.networks["net"] = NN.Network([63, 63, 26])
		else:
			print("Invalid type")
			sys.exit(1)
	...
	def train(self, fileName):
		...
		if self.type == "mono":
			...
			iterator = 0
			for item in data:
				trainingData.append((item, iterator))
				iterator += 1
			self.networks["net"].SGD(trainingData, 1000, 10, 0.1)
	...
	def predict(self, data):
		if self.type == "mono":
			...
			for guess in self.networks["net"].feedforward(data):
...
if __name__ == "__main__":
	#g = Guesser(type="mono", name="monoNet")
	#g.train('json_info.txt')
	#g.save()
	g = pickle.load(open('monoNet', 'rb'))
	rando = []
