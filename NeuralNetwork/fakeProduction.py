...
def processAndPredict():
	...
	for i in range(len(letters)):
		...
		combo = letters[i][5] + letters[i][6] + letters[i][7] + letters[i][8]
		if len(combo) > 0:
			first = max(set(combo), key = combo.count)
			lessCombo = list(filter(lambda a: a != first, combo))
			if len(lessCombo) == 0:
				collection.append([first])
			else:
				second = max(set(lessCombo), key = lessCombo.count)
